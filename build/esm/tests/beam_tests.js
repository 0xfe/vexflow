...
function complexWithArticulation(options) {
    ...
    const notes1 = s1.map((struct) => f.StaveNote(struct).addModifier(0, f.Articulation({ type: 'am', position: 'above' })));
    const notes2 = s2.map((struct) => f.StaveNote(struct).addModifier(0, f.Articulation({ type: 'a>', position: 'below' })));
    f.Beam({ notes: notes1 });
    f.Beam({ notes: notes2 });
