...
const GhostNoteTests = {
  Start(): void {
    ...
    run('GhostNote Basic', this.basic);
    run('GhostNote Dotted', this.dotted);
  },

  basic: createTest((f: Factory, score: EasyScore) => {
  ...
  dotted: createTest((f: Factory, score: EasyScore) => {
...
function createTest(setup: (f: Factory, es: EasyScore) => void) {
