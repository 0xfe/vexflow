...
VF.Test.Tuplet = (function() {
  var Tuplet = {
    ...
    setupContext: function(options, x, y) {
      ...
      var stave = new VF.Stave(10, 10, x || 350)
        .addTrebleGlyph()
        .setContext(ctx)
        .draw();
    ...
    simple: function(options, contextBuilder) {
      ...
      var tuplet1 = vf.Tuplet({ notes: notes.slice(0, 3) });
      var tuplet2 = vf.Tuplet({ notes: notes.slice(3, 6) });
    ...
    beamed: function(options, contextBuilder) {
      ...
      var beam1 = vf.Beam({ notes: notes.slice(0, 3) });
      var beam2 = vf.Beam({ notes: notes.slice(3, 10) });

      var tuplet1 = vf.Tuplet({ notes: notes.slice(0, 3) });
      var tuplet2 = vf.Tuplet({ notes: notes.slice(3, 10) });
    ...
    ratio: function(options, contextBuilder) {
      ...
      var beam = vf.Beam({ notes: notes.slice(3, 6) });
      var tuplet1 = vf.Tuplet({ notes: notes.slice(0, 3) });
      var tuplet2 = vf.Tuplet({
    ...
    awkward: function(options, contextBuilder) {
      ...
      var beam = vf.Beam({ notes: notes.slice(0, 11) });
      var tuplet1 = vf.Tuplet({
    ...
    complex: function(options, contextBuilder) {
      ...
      var beam1 = vf.Beam({ notes: notes1.slice(0, 3) });
      var beam2 = vf.Beam({ notes: notes1.slice(5, 9) });
      var beam3 = vf.Beam({ notes: notes1.slice(11, 16) });
