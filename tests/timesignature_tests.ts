...
const TimeSignatureTests = {
  Start(): void {
    ...
    test('Time Signature Parser', this.parser);
    const run = VexFlowTests.runTests;
    run('Basic Time Signatures', this.basic);
    run('Big Signature Test', this.big);
    run('Time Signature multiple staves alignment test', this.multiple);
    run('Time Signature Change Test', this.change);
  },
  ...
  parser(): void {
    ...
    mustFail.forEach((invalidString) => throws(() => timeSig.parseTimeSpec(invalidString), /BadTimeSignature/));
  ...
  big(options: TestOptions, contextBuilder: ContextBuilder): void {
  ...
  change(options: TestOptions): void {
