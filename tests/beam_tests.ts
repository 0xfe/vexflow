...
function autoStem(options: TestOptions): void {
  ...
  const notes = voice.getTickables() as Note[];
...
function mixed(options: TestOptions): void {
  ...
  [
    ...
    [4, 8],
    [8, 12],
  ].forEach((range) => f.Beam({ notes: voice1.getTickables().slice(range[0], range[1]) as Note[] }));

  [
    [0, 4],
    [4, 8],
    [8, 12],
  ].forEach((range) => f.Beam({ notes: voice2.getTickables().slice(range[0], range[1]) as Note[] }));
...
function mixed2(options: TestOptions): void {
  ...
  f.Beam({ notes: voice.getTickables().slice(0, 12) as Note[] });
  f.Beam({ notes: voice2.getTickables().slice(0, 12) as Note[] });
